    ...
    return new Promise<AssetMetadata>(async (resolve, reject) => {
      try {
        if (this.chunkSize === undefined) {
          const metadata = await this.configProvider.get(
            "chunk.size",
            ConfigTypeEnum.CONFIG_TYPE_SYSTEM,
          );
        ...
        const asset = create(AssetMetadataSchema, {
          ...
          createdAt: toISO(
            convertToUtcDateTime(
              convertDateToDateTime(new Date(file.lastModified)),
            ),
          ),
        ...
        if (parentHash) {
          const parent = await this.assetProvider.detail(
            namespaceId,
            parentHash,
          );
        ...
        for (let index = 0; index < asset.chunkCount; index++) {
          ...
          console.log(
            `Try to read the index=${index} chunk, size=${end - start}`,
          );
