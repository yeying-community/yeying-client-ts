...
export class NamespaceProvider {
  ...
  async getDefaultNamespace() {
    try {
      const configMetadata = await this.configProvider.get(
        "namespace.default",
    ...
    return new Promise<NamespaceMetadata[]>(async (resolve, reject) => {
      ...
      try {
        header = await this.authenticate.createHeader(
          toBinary(SearchNamespaceRequestBodySchema, body),
        );
      } catch (err) {
        console.error(
          `Fail to create header when searching namespace, page=${page}, pageSize=${pageSize}`,
          err,
        );
        return reject(err);
      }
      ...
      try {
        ...
        for (const n of resBody.namespaces) {
          try {
            ...
            console.error(
              `Fail to verify namespace=${JSON.stringify(toJson(NamespaceMetadataSchema, n))}`,
            );
          }
        }
  ...
  create(
    ...
    return new Promise<NamespaceMetadata>(async (resolve, reject) => {
      ...
      try {
        ...
        header = await this.authenticate.createHeader(
          toBinary(CreateNamespaceRequestBodySchema, body),
        );
      } catch (err) {
