...
export class BlockProvider {
    ...
    return new Promise<BlockDetail>(async (resolve, reject) => {
      ...
      try {
        header = await this.authenticate.createHeader(
          toBinary(GetBlockRequestBodySchema, body),
        );
      } catch (err) {
        console.error(
          `Fail to create header when getting chunk content, hash=${hash}`,
          err,
        );
        return reject(err);
      }
  ...
  confirm(block: BlockMetadata) {
    return new Promise<BlockMetadata | undefined>(async (resolve, reject) => {
      ...
      try {
        header = await this.authenticate.createHeader(
          toBinary(ConfirmBlockRequestBodySchema, body),
        );
      } catch (err) {
  ...
  put(namespaceId: string, data: Uint8Array) {
    return new Promise<BlockMetadata>(async (resolve, reject) => {
      ...
      try {
        header = await this.authenticate.createHeader(
          toBinary(PutBlockRequestBodySchema, body),
        );
      } catch (err) {
      ...
      try {
        ...
        console.error(
          `Fail to put block=${JSON.stringify(toJson(BlockMetadataSchema, block))}`,
          err,
        );
        return reject(err);
      }
