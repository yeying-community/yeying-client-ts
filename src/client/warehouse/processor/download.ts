...
export class DownloadProcessor implements Processor {
  ...
  async initialize(c: CommandMessage): Promise<ProcessMessage> {
    ...
    this.downloader = new Downloader(
      config.providerOption,
      config.securityAlgorithm,
    );
  ...
  async start(c: CommandMessage): Promise<ProcessMessage> {
    ...
    const complete = (a: any) =>
      this.callback(this.createProcessMessage(c, "COMPLETE", a));
    const error = (e: any) =>
      this.callback(this.createProcessMessage(c, "ERROR", e.message));
    const progress = (b: any) =>
      this.callback(this.createProcessMessage(c, "PROGRESS", b), [
        b.data.buffer,
      ]);
    this.downloader
      ?.download(message.namespaceId, message.hash, progress)
      .then(complete)
      .catch(error);
