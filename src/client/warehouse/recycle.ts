...
export class RecycleProvider {
    ...
    return new Promise<DeletedAssetMetadata[]>(async (resolve, reject) => {
      ...
      try {
        header = await this.authenticate.createHeader(
          toBinary(SearchDeletedAssetRequestBodySchema, body),
        );
      } catch (err) {
        console.error(
          "Fail to create header for searching deleted assets",
          err,
        );
        return reject(err);
      }
  ...
  recover(namespaceId: string, hash: string) {
    return new Promise<void>(async (resolve, reject) => {
      ...
      try {
        header = await this.authenticate.createHeader(
          toBinary(RecoverDeletedAssetRequestBodySchema, body),
        );
      } catch (err) {
  ...
  remove(namespaceId: string, hash: string) {
    return new Promise<void>(async (resolve, reject) => {
      ...
      try {
        header = await this.authenticate.createHeader(
          toBinary(RemoveDeletedAssetRequestBodySchema, body),
        );
      } catch (err) {
        console.error("Fail to create header when removing from recycle", err);
        return reject(err);
