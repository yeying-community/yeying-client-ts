...
export class SupportProvider {
  ...
  async collectFaq(type: string, email: string, description: string) {
    return new Promise<FaqMetadata>(async (resolve, reject) => {
      ...
      try {
        faq.signature = await this.authenticate.sign(
          toBinary(FaqMetadataSchema, faq),
        );
        body.data.value = faq;
        body.data.case = "faq";
        header = await this.authenticate.createHeader(
          toBinary(CollectSupportRequestBodySchema, body),
        );
      } catch (err) {
      ...
      const request = create(CollectSupportRequestSchema, {
        header: header,
        body: body,
      });
      try {
