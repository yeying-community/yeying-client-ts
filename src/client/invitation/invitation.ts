...
import {
  signInvitationMetadata,
  verifyInvitationMetadata,
} from "../model/model";
import {
  formatDateTime,
...
export class InvitationProvider {
  ...
  create(duration: number, invitee?: string) {
    return new Promise<InvitationMetadata>(async (resolve, reject) => {
      const invitation: InvitationMetadata = create(InvitationMetadataSchema, {
        ...
        expiredAt: toISO(
          plusSecond(getCurrentUtcDateTime(), duration * 24 * 3600),
        ),
      });
      ...
      try {
        ...
        header = await this.authenticate.createHeader(
          toBinary(CreateInvitationRequestBodySchema, body),
        );
      } catch (err) {
  ...
  search(page: number, pageSize: number) {
    return new Promise<InvitationMetadata[]>(async (resolve, reject) => {
      ...
      try {
        header = await this.authenticate.createHeader(
          toBinary(SearchInvitationRequestBodySchema, body),
        );
      } catch (err) {
  ...
  detail(code: string) {
    return new Promise<InvitationMetadata>(async (resolve, reject) => {
      ...
      try {
        header = await this.authenticate.createHeader(
          toBinary(InvitationDetailRequestBodySchema, body),
        );
      } catch (err) {
        console.error(
          "Fail to create header for getting invitation detail.",
          err,
        );
        return reject(err);
      }
