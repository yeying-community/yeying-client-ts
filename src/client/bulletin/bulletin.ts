    ...
    return new Promise<SolutionMetadata[]>(async (resolve, reject) => {
      ...
      try {
        ...
        header = await this.authenticate.createHeader(
          toBinary(BulletinListRequestBodySchema, body),
        );
      } catch (err) {
        console.error(
          "Fail to create header for BulletinListing solutions.",
          err,
        );
        return err;
      }

      const request = create(BulletinListRequestSchema, {
        header: header,
        body: body,
      });
      try {
        ...
        for (let solution of body.solutions) {
          try {
            ...
            console.error(
              `Invalid solution=${JSON.stringify(toJson(SolutionMetadataSchema, solution))}`,
            );
          }
        }
