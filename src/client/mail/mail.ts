    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      try {
        ...
        header = await this.authenticate.createHeader(
          toBinary(SendMailRequestBodySchema, body),
        );
      } catch (err) {
      ...
      const request = create(SendMailRequestSchema, {
        header: header,
        body: body,
      });
      try {
  ...
  verify(toMail: string, code: string) {
    return new Promise<void>(async (resolve, reject) => {
      ...
      try {
        // 创建消息头
        header = await this.authenticate.createHeader(
          toBinary(VerifyMailRequestBodySchema, body),
        );
      } catch (err) {
      ...
      const request = create(VerifyMailRequestSchema, {
        header: header,
        body: body,
      });
      try {
