...
export class ProviderProvider {
  ...
  descriptions() {
    return new Promise<ProviderDescription[]>(async (resolve, reject) => {
      ...
      const request = create(ProviderDescriptionsRequestSchema, {
        header: header,
      });
      try {
  ...
  add(name: string, code: ProviderCodeEnum, key: string, uid?: string) {
    return new Promise<ProviderMetadata>(async (resolve, reject) => {
      ...
      try {
        ...
        header = await this.authenticate.createHeader(
          toBinary(AddProviderRequestBodySchema, body),
        );
      } catch (err) {
      ...
      const request = create(AddProviderRequestSchema, {
        header: header,
        body: body,
      });
      try {
  ...
  search(page: number, pageSize: number, code?: ProviderCodeEnum) {
    return new Promise<ProviderMetadata[]>(async (resolve, reject) => {
      ...
      try {
        header = await this.authenticate.createHeader(
          toBinary(SearchProviderRequestBodySchema, body),
        );
      } catch (err) {
      ...
      const request = create(SearchProviderRequestSchema, {
        header: header,
        body: body,
      });
      try {
  ...
  delete(uid: string) {
    return new Promise<void>(async (resolve, reject) => {
      ...
      try {
        header = await this.authenticate.createHeader(
          toBinary(DeleteProviderRequestBodySchema, body),
        );
      } catch (err) {
  ...
  detail(uid: string) {
    return new Promise<ProviderDetail>(async (resolve, reject) => {
      ...
      try {
        header = await this.authenticate.createHeader(
          toBinary(ProviderDetailRequestBodySchema, body),
        );
      } catch (err) {
        console.error(
          "Fail to create header for getting provider detail.",
          err,
        );
        return reject(err);
      }

      const request = create(ProviderDetailRequestSchema, {
        header: header,
        body: body,
      });
      try {
