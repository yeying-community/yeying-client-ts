...
export function convertApplicationMetadataFromIdentity(identity: Identity) {
  ...
  return create(ApplicationMetadataSchema, {
    ...
    serviceCodes: extend.serviceCodes
      .split(",")
      .map((a) => ServiceCodeEnum[a as keyof typeof ServiceCodeEnum]),
    hash: extend.hash,
...
export function convertServiceMetadataFromIdentity(identity: Identity) {
  ...
  return create(ServiceMetadataSchema, {
    ...
    apiCodes: extend.apiCodes
      .split(",")
      .map((a) => ApiCodeEnum[a as keyof typeof ApiCodeEnum]),
    proxy: extend.proxy,
    grpc: extend.grpc,
    avatar: metadata.avatar,
    createdAt: metadata.createdAt,
